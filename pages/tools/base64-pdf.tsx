...
const Convert: React.FC = () => {
    ...
    const convertToImage = () => {
        try {
            if (base64String) {
                const image = `data:application/pdf;base64,${base64String}`;
                setImageSrc(image);
                setError('');
            } else {
    ...
    const downloadImage = () => {
        if (imageSrc) {
            ...
            link.href = imageSrc;
            link.download = 'converted_pdf.pdf'; // Changed to PDF
    ...
    return (
        <div className="py-2">
            <Head>
                <title>Base64 to PDF Converter | freetoolsarebest </title>
                <link rel="icon" href="/favicon.ico" />
            </Head>
            <Header />
            <div className='p-4 lg:px-[5.5rem] md:px-10 flex flex-col items-center justify-center'>
                <h1 className="text-3xl font-bold text-center mt-5 mb-5">Base64 to PDF Converter</h1>
                ...
                {imageSrc && (
                    <div className="mt-8">
                        <h2 className="text-xl font-semibold mb-2">Converted PDF:</h2>
                        <button
                        ...
                        <iframe
                            className="mt-2 lg:w-[60rem] lg:h-[60rem] h-64 border"
                            src={imageSrc}
                            title="PDF Preview"
                        ></iframe>
                    </div>
