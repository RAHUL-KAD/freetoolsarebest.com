...
const Convert: React.FC = () => {
    ...
    const convertToImage = () => {
        try {
            if (base64String) {
                const image = `data:image/webp;base64,${base64String}`; // Changed to WebP
                setImageSrc(image);
                setError('');
            } else {
    ...
    const downloadImage = () => {
        if (imageSrc) {
            ...
            link.href = imageSrc;
            link.download = 'converted_image.webp'; // Changed to WebP
    ...
    return (
        <div className="py-2">
            <Head>
                <title>Base64 to WebP Converter | freetoolsarebest </title> {/* Updated title */}
                <link rel="icon" href="/favicon.ico" />
            </Head>
            <Header />
            <div className='p-4 lg:px-[5.5rem] md:px-10 flex flex-col items-center justify-center'>
                <h1 className="text-3xl font-bold text-center mt-5 mb-5">Base64 to WebP Converter</h1> {/* Updated heading */}
                ...
                <div className="mt-4 flex items-center justify-center gap-5 ">
                    ...
                    >
                        Convert to WebP {/* Updated button text */}
                    </button>
                </div>
                ...
                {imageSrc && (
                    <div className="mt-8">
                        <h2 className="text-xl font-semibold mb-2">Converted WebP:</h2> {/* Updated heading */}
                        <button
                            className="mt-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded"
                            onClick={downloadImage}
                        >
                            Download WebP {/* Updated button text */}
                        </button>
